...
export default function AcercaDe({ navigation }) {
  ...
  const handleBarCodeScanned = ({ type, data }) => {
    setScanned(true);
    alert(`El codigo de barras "${data}" ha sido escaneado`);
  };
          ...
          <ImageBackground source={{ uri: image }} style={styles.image}>
            ...
            >
              <Text style={{ fontSize: 20 }}>
                Federico Gomboc y Luciano Neiman
              </Text>
              <Text style={{ fontFamily: "barcodeFont", fontSize: 40 }}>
                Federico Gomboc y Luciano Neiman
              </Text>
            </TouchableOpacity>
            <TouchableOpacity style={{ backgroundColor: "white" }}>
              <Text style={{ fontSize: 20 }}>
                Presione el QR para copiar el texto
              </Text>
              <Button
